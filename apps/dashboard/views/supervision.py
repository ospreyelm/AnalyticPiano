...
def supervisors_view(request):
    ...
    approved_supervisors = [
        x for x in request.user.supervisors if request.user.is_subscribed_to(x)
    ]

    supervisors_table = SupervisorsTable(
        [{"supervisor": x, "user": request.user} for x in visible_supervisors]
    )
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            request.user.subscribe_to(
                supervisor, status=User.SUPERVISOR_STATUS_SUBSCRIPTION_WAIT
            )
...
def supervisors_courses_view(request):
    ...
    supervisors_courses = Course.objects.filter(
        Q(visible_to__members__id__contains=request.user.id) | Q(visible_to=None),
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            subscriber.subscribe_to(
                request.user, status=User.SUPERVISOR_STATUS_INVITATION_WAIT
            )
