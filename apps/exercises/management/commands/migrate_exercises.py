...
class Command(BaseCommand):
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS('%s playlists are successfully created.' % Playlist.objects.count()))
        ...
        for repo_exercise in repository.exercises:
            ...
            playlist = Playlist.objects.filter(name=repo_exercise.getGroupName()).first()
        ...
        self.stdout.write(
            self.style.SUCCESS(
                '%s exercises are successfully created and assigned to proper playlists.' % Exercise.objects.count()
            )
        )
