...
class BaseContentResource(resources.ModelResource):
    authored_by = Field(attribute='authored_by__email',
                        column_name='authored_by')
    ...
    def get_export_fields(self):
        ...
        if self.is_sample:
            export_fields = [field for field in export_fields
...
class ExerciseResource(BaseContentResource):
    class Meta(BaseContentResource.Meta):
        model = Exercise
        fields = ('id', 'description', 'data', 'rhythm',
                  'time_signature', 'is_public', 'authored_by',
                  'locked', 'created', 'updated')
        export_order = fields
        sample_import_file_fields = ('description', 'data', 'rhythm',
                                     'time_signature', 'is_public', 'locked')


class PlaylistResource(BaseContentResource):
    class Meta(BaseContentResource.Meta):
        model = Playlist
        fields = ('id', 'name', 'exercises', 'transpose_requests', 'transposition_type',
                  'is_public', 'is_auto', 'authored_by', 'created', 'updated')
        export_order = fields
        sample_import_file_fields = ('name', 'exercises', 'transpose_requests',
                                     'transposition_type', 'is_public')


class CourseResource(BaseContentResource):
    class Meta(BaseContentResource.Meta):
        model = Course
        fields = ('id', 'title', 'slug', 'playlists', 'is_public',
                  'publish_dates', 'due_dates', 'authored_by', 'created', 'updated')
        export_order = fields
        sample_import_file_fields = ('title', 'slug', 'playlists', 'is_public',
                                     'publish_dates', 'due_dates')
