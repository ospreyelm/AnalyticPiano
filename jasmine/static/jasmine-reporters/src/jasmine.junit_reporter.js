(function () {
  ...
  var JUnitXmlReporter = function (savePath, consolidate, useDotNotation) {
    ...
    this.useDotNotation =
      useDotNotation === jasmine.undefined ? true : useDotNotation;
  };
      ...
      for (var i = 0; i < resultItems.length; i++) {
        ...
        if (result.type == "expect" && result.passed && !result.passed()) {
          ...
          failure +=
            ...
            result.type +
            '" message="' +
            trim(escapeInvalidXmlChars(result.message)) +
            '">';
          failure += escapeInvalidXmlChars(
            result.trace.stack || result.message
          );
          failure += "</failure>";
        }
      }
