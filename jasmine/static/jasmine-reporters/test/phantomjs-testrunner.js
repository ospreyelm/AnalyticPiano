...
if (phantom.args.length === 0) {
  ...
  console.log("");
  console.log(
    "NOTE: This script depends on jasmine.TrivialReporter being used\non the page, for the DOM elements it creates.\n"
  );
  phantom.exit(2);
} else {
...
function logAndWorkAroundDefaultLineBreaking(msg) {
  var interpretAsWithoutNewline =
    /(^(\033\[\d+m)*[\.F](\033\[\d+m)*$)|( \.\.\.$)/;
  if (
    navigator.userAgent.indexOf("Windows") < 0 &&
    interpretAsWithoutNewline.test(msg)
  ) {
    var fs = require("fs");
  ...
  if (status !== "success") {
    ...
    var isFinished = function () {
      return page.evaluate(function () {
        ...
        return (
          document.getElementsByClassName("finished-at").length &&
    ...
    var getResults = function () {
      return page.evaluate(function () {
        return (
          (document.getElementsByClassName("description").length &&
            document
              .getElementsByClassName("description")[0]
              .innerHTML.match(/(\d+) spec.* (\d+) failure.*/)) || [
    ...
    var ival = setInterval(function () {
      if (isFinished()) {
        ...
        for (var filename in xml_results) {
          if (
            xml_results.hasOwnProperty(filename) &&
            (output = xml_results[filename]) &&
