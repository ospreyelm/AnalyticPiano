define(["lodash", "app/models/keyboard_generator"], function (
  _,
  KeyboardGenerator
) {
  describe("KeyboardGenerator", function () {
    ...
    it("should return the starting MIDI note for keyboard sizes", function () {
      ...
      _.each(sizes, function (size) {
        var kg = new KeyboardGenerator(size);
        expect(kg.startingNote(size)).toBe(
          KeyboardGenerator.startingNoteForSize[size]
        );
      });
    });
    ...
    it("should generate valid key specs", function () {
      ...
      expect(kg.keySpecs[kg.keySpecs.length - 1].noteNumber).toBe(
        kg.startingNote(size) + size - 1
      );
