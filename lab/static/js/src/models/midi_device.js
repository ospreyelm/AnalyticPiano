define(["lodash", "microevent", "Tone" /* Tone.js 14.8.3 */], function (
  _,
  MicroEvent,
  Tone
) {
  ...
  if ($("#volume").length > 0) {
    $("#volume").click(function () {
      ...
      volumeDiv.innerHTML =
        volumeOptsKeys[
          (1 + volumeOptsKeys.indexOf(volumeDiv.innerHTML)) %
            volumeOptsKeys.length
        ];
  ...
  MidiDevice.prototype.selectDefaults = function (input) {
    this.selectInput(
      0,
      input
    ); /* previously selectInput(0), now sending all inputs*/
