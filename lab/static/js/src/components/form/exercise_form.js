define([
  ...
  ExerciseFormComponent.prototype.validate = function (data) {
    ...
    var highlight_error = function (el) {
      $(el)
        .css({ "border-color": "red", "background-color": "#f2dede" })
    ...
    if (!/<[^>]+>/.test(data.lilypond_chords)) {
      ...
      this.notify({
        title: "Error: Chords",
        description:
          "Please enter at least one chord enclosed in angle brackes.",
        type: "error",
      });
    ...
    $.ajax({
      ...
      statusCode: {
        403: function () {
          that.notify({
            ...
            description:
              "<p>You must be logged in as an instructor or administrator to submit an exercise.</p>",
            type: "error",
          });
        },
        500: function () {
          that.notify({
            title: "Server Error",
            description:
              "<p>An error ocurred on the server while attempting to save the exercise.</p>",
            type: "error",
          });
        },
      },
        ...
        if ("status" in response) {
          if (response.status == "success") {
            that.notify({
              ...
              description:
                'Exercise saved successfully! <a href="' +
                response.data.url +
                '">Click to view exercise</a>.',
              type: "success",
            });
            ...
            that.notify({
              title: "Error",
              description:
                "<p>" + response.message + "</p><p>" + errors + "</p>",
              type: "error",
            });
          }
        } else {
      ...
      .fail(function (jqXHR, textStatus) {
        that.notify({
          title: "Error",
          description:
            "There was an error saving the exercise (" + textStatus + ").",
          type: "error",
        });
      });
  ...
  ExerciseFormComponent.prototype.collectFormData = function () {
    ...
    var key_signature =
      this.widgets.keySignature.keySignature.getSignatureSpec();
    ...
    var new_exercise_group = this.cleanGroupName(
      this.$new_exercise_group.val()
    );
