define([
  ...
  AppManageComponent.prototype.renderExerciseList = function (data) {
    ...
    if (groups.length == 0) {
      ...
      $el.html("");
      $el.append(
        '<div class="exercise-collapse-all"><span class="btn btn-toggle"><i class="ion-arrow-down-b" data-toggle-cls="ion-arrow-down-b ion-arrow-right-b" data-toggle-all="yes"> Collapse All</i></span></div>'
      );
      $.each(groups, function (i, group) {
        ...
        var $li = $(
          [
            ...
            '<a class="btn btn-group" href="' +
              group.url +
              '" target="_blank">Group: <b>' +
              group.name +
              "</b> (" +
              num_exercises +
              ")</a>",
            '<div class="actions" style="display:none">',
            '<i class="ion-close-circled btn btn-delete" data-group-name="' +
              group.name +
              '"></i>',
            "</div>",
            "</div>",
        ...
        if (num_exercises > 0) {
          ...
          $.each(group.data.exercises, function (j, exercise) {
            var $childLi = $(
              [
                '<li class="js-row">',
                '<a class="btn btn-exercise" href="' +
                  exercise.url +
                  '" target="_blank">Exercise: <b>' +
                  exercise.name +
                  "</b></a>",
                '<div class="actions" style="display:none">',
                '<i class="ion-close-circled btn btn-delete" data-exercise-name="' +
                  exercise.name +
                  '" data-group-name="' +
                  group.name +
                  '"></i>',
