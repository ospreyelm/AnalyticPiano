define(["lodash", "jquery", "app/components/component"], function (
  _,
  $,
  Component
) {
  ...
  _.assign(KeyboardInputComponent.prototype, {
        ...
        if (keyMap.hasOwnProperty(keyName)) {
          ...
          if (!codeFor.hasOwnProperty(keyName)) {
            errors.push(
              "keyMap key [" +
                keyName +
                "] is invalid, does not have a key code"
            );
          } else if (
            !(
              keyObj.hasOwnProperty("msg") &&
              typeof keyObj.msg === "string" &&
              keyObj.msg.length > 0
            )
          ) {
            errors.push(
              "keyMap key [" + keyName + "] is missing 'msg' property"
            );
          }
        }
      }
    ...
    isModifierKey: function (keyEvent) {
      return keyEvent.altKey || keyEvent.ctrlKey || keyEvent.metaKey
        ? true
        : false;
    },
    /**
